...
Official `terminal/follow` and `terminal/retain` Remote streams require the account SSH grant and Session ownership. This does not authorize shared default-model initialization or unknown terminal endpoints.
...
- The same page creates folders and moves or copies entries into another directory (choose Move or Copy on a row, open the target folder, then paste). The source must be a plain file or directory and the destination must not exist yet inside the caller's managed folder, so nothing is overwritten and no symlink can write outside it
- The page can also `git clone` a repository and run `git pull --ff-only` on an already cloned directory. Only http/https URLs without query parameters are accepted; private repositories use separate username and password/token fields (a PAT for GitHub) and both the upload and the git permission must be granted. Git runs with a minimal environment: no host git configuration or credential helper, no interactive prompt, no transport other than http(s) (which excludes command-executing transports such as `ext::`), and repository symlinks are written as plain files. One git task per account at a time, a five-minute timeout, and credentials are masked in the returned output. Credentials are used for one clone or pull and cleared from the inputs on submission; they are not stored in the repository URL or a credential store. Enter them again for a later private pull. Authenticated requests do not follow redirects: use the final repository URL without query parameters. New clones extract legacy URL userinfo and save a credential-free origin; existing repository credentials are not bulk-migrated
...
- **Workspace and session permissions**: the owner enables workspaces per subuser with switches; enabled workspaces expose active sessions by default, with per-session checkboxes to turn individual sessions off. A subuser may remove registrations backed by that account's managed directory or paired local computer, but not a shared grant or another account's private directory. Removal moves that user's sessions to Ungrouped without deleting directories, files, pairing records, or Session logs; the Sessions remain readable while their directory stays authorized. New blank sessions are assignable before their first prompt and display their session ID when untitled. Archived or deleted sessions and workspaces whose directories are unavailable are excluded from the settings list; failed reads show an error asking the owner to retry
...
- Windows workspaces register `office_native_status` plus `*_native_read` and `*_native_edit` for Word, Excel, and PowerPoint: they prefer the Microsoft Office applications installed on the customer computer and fall back to WPS Writer, Spreadsheets, and Presentation without any `@univerjs-pro/*` dependency
...
Windows local workspaces also expose native Word, Excel, and PowerPoint automation. `word_native_read` reads body text, paragraphs, and tables. `word_native_edit` creates documents or applies a batch of find/replace, paragraph insertion/deletion/formatting, tables, headers, footers, images, page breaks, and PDF exports. `excel_native_read` reads each worksheet's used range and returns date cells as ISO timestamps; `excel_native_edit` writes cell blocks, formulas, formatting, column widths and row heights, manages worksheets, inserts images, replaces text, and exports PDF, treating a cell value that starts with `=` as a formula. `ppt_native_read` reads shape text and speaker notes per slide; `ppt_native_edit` adds and deletes slides, sets placeholder and textbox text, notes, and images, replaces text, and exports PDF, using the shape indexes the read reports. The companion tries `Word.Application`, `Excel.Application`, and `PowerPoint.Application` first and WPS `kwps.Application`, `ket.Application`, and `kwpp.Application` second; `office_native_status` reports availability for all three. Each action loads only its own PowerShell script. Arguments reach a fixed PowerShell script as JSON over stdin instead of command interpolation; every document, image, and export path must stay under the authorized folder. The document is saved only after the complete batch succeeds, and macro automation is disabled while opening it.

Office RPC requires local-companion protocol v2. Excel and PowerPoint are new actions inside that protocol rather than a version bump, so an already-paired older companion still connects but rejects the Excel and PowerPoint tools with an unsupported-action error, and its `office_native_status` result omits the `apps` field. After upgrading the server plugin, download and run the new EXE again.
...
- **npm fails installing dsh (allow-scripts / node-pty)?** Newer npm blocks install scripts. Allow them first, then reinstall: `npm config set allow-scripts=@deepseek-ai/dsh-subprocess-local,koffi,node-pty,@google/genai,protobufjs --location=user` followed by `npm install -g @deepseek-ai/dsh@0.1.3-alpha.1` again (this project itself has no such issue — it's dsh's dependencies that run native builds).
...
- **Subuser isolation (third-party plugin surface)**: SSH uses account permissions and private connection storage; shared administration endpoints such as skin-center, modlens, and the dsh-uploads list/delete remain owner-only. The Workshop `/api/preset-center` path and its descendants are also administrator-only, covering shared preset listings, composition source, enable, disable, and uninstall. Ordinary accounts can still list and select their authorized session presets; SSH endpoint grants cannot authorize global preset management. Upload/download stay gated by `allow_upload` / `allowGitDownload`, and **new subusers default to git download off** (including SSH remote file download and dsh-uploads download); directory listing in account-isolated SSH requires only SSH permission. Ordinary better-sidebar previews and downloads are separately bound to a Session owned by the current account and its authorized workspace; forged `sessionId`, `cwd`, or file paths receive 403. The gateway authorizes and reads each subuser file through the same open descriptor, so a concurrent path replacement cannot redirect the read to another account's file.
...
- Targets DeepSeek Harness `0.1.3-alpha.1`, with runtime peers and compiler dependencies pinned to that exact version. The optional principal-access peer, installation guidance, and Docker defaults use the same release.
...
Restricted sidebar terminals mount only the authenticated account's managed workspace at `/workspace`. The Host verifies the durable session owner and directory on every attach, rejects foreign sessions, directory escapes and shared agent PTY UUIDs, and starts a shell without service secrets. Private mount and PID namespaces isolate files and processes. The shell runs as the service's ordinary OS user with no capabilities and the `dsh-sandbox` group; the deployment firewall restricts that group's network access while permitting dependency downloads. Administrators retain their existing terminal. Disconnected terminals expire after the configured grace period.
...
Enable `MCP_TENANT_AGENT_SHELL=true` to give managed-workspace agents and subagents a `bash` tool with terminal-card results. Each call rechecks the account and write permission. Only the current project and personal HOME are writable; other accounts remain inaccessible. HOME is shared with the web terminal and editor, so user-level Node and pnpm installations persist. Cancellation and timeout terminate the command. Deploy the updated launcher and its sudoers digest together; see the [execution and validation rules](docs/plans/2026-09-11-tenant-agent-shell.md).
...
Restricted accounts can subscribe to file changes in their own session workspace through `workspaceFiles/changes`. Opening or closing a delivered-file preview preserves history streams on the same connection. The gateway checks session ownership, allowed folders, and disabled status when opening the subscription and before forwarding changes; invalid or foreign session requests are rejected.
...
### Independent SSH for each account

Install an account-isolated `dsh-ssh` Host, enable its Cordis `accountIsolation: true` setting, and set `TENANT_SSH_ENABLED=true` in the `.env` shared by the gateway and password plugin. This switch defaults to disabled and must not be enabled with an older shared Host. The gateway signs the logged-in identity for HTTP and terminal WebSocket requests; the Host calls `principalAccess.sshAccess` before each browser operation and model tool execution to validate the account and its SSH permission.

Users add their own connections from the sidebar SSH page using a password or pasted private key. They can edit and delete connections, execute commands, open terminals, transfer files, and manage tunnels independently. Different accounts may reuse the same alias; host lists, connection pools, jump-host references, cluster execution, and tunnels stay within the current account. Restricted accounts cannot import the server's `~/.ssh/config`, use server private-key paths or an SSH agent, or invoke model upload/download tools with local Host paths; transfers use browser upload/download instead.

Accounts created with `TENANT_SSH_ENABLED=true` have SSH enabled by default, and administrators can disable it in the account list's permissions dialog; older shared Host deployments do not grant it automatically. Existing explicit settings and omitted fields in partial updates retain their values; missing permission rows deny access. Remote directory listing requires only SSH permission; upload additionally requires upload permission, and file download additionally requires download permission. Disabling SSH, logging out, changing credentials, banning, or deleting an account disconnects its active browser terminals without affecting other accounts.
...
The directory contains `manifest.json` with `version`, a 40-character source `commit`, and `files`. Each entry has `file`, `platform` (`windows-x64` or `mac-arm64`), `bytes`, `sha256`, and optional `signing` (`unsigned` or Mac-only `apple-notarized`; omission displays unsigned). Mark Mac artifacts `apple-notarized` only after signing, notarization, and installation verification pass. Only simple `.exe`, `.dmg`, and `.zip` filenames are published; directory listings and unlisted files are unavailable. Verify uploaded SHA-256 hashes before switching configuration and restarting the gateway. Published releases must remain read-only; do not replace files in place. Downloads support HEAD and Range. An empty setting disables these public routes.
