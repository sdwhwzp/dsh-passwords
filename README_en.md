...
- The page can also `git clone` a repository and run `git pull --ff-only` on an already cloned directory. Only http/https URLs are accepted (put the access token in the URL for a private repository) and both the upload and the git permission must be granted. Git runs with a minimal environment: no host git configuration or credential helper, no interactive prompt, no transport other than http(s) (which excludes command-executing transports such as `ext::`), and repository symlinks are written as plain files. One git task per account at a time, a five-minute timeout, and credentials are masked in the returned output
