...
服务器 30 已能独立提供 DSH 服务，入口 `http://192.168.10.30:3081/`，用户库跑在 MariaDB。服务器 28 全天在线未中断，仍是对外的权威实例，公网 `wh.gr-iot.cn:3081` 未改指向。两台机器现在共用同一份 root 启动器源码。
...
**28 尚未迁 MariaDB。** 28 仍连 MySQL `192.168.10.95` 正常服务。它不应沿用 30 上的 `2.6.28`：该版本裹了尚未上过生产的 `2.6.27`（账号管理 UI 重做与托管目录的 git clone/pull），数据库迁移不该与未验收的 UI 变更捆在一次变更里。28 应另出「线上 2.6.26 + 排序规则修改」的最小包，并走完整的候选 Profile 装配冒烟与切换流程。
