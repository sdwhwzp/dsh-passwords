...
管理员通过 `dev_server_admin` 列出子账号服务、查看状态并停止指定账号的服务；停止同时禁用自启动。工具仅在已验证的管理员步骤中注册，执行时再次检查当前身份及封禁状态，并记录停止操作审计。管理工具不返回启动命令或应用日志；普通账号不能借此管理其他账号服务。
...
单次 `bash` 保持前台命令的取消、超时与进程清理规则；不得通过移除 PID 隔离或 `--die-with-parent` 来保留后台作业。持久服务的 unit 只执行现有租户启动器，启动器再进入当前账号的沙箱和项目。命令文本从 root 所有的文件经 stdin 传入，不能拼接为 systemd 指令。用户名称、服务名称、端口、当前身份、会话写入权限与规范化目录都经过检查；其他账号或其他项目不能查询、修改或停止该服务。账号停用后拒绝新的工具操作，已运行服务由管理员处置，不能把禁用账号等同于自动停止进程。
