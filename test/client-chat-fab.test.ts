...
async function mountChat(t: TestContext) {
  ...
  const middleDown = () => ({
    button: 1,
    isPrimary: true, pointerId: 1, currentTarget: { setPointerCapture() {} },
    clientX: 50,
    clientY: 50,
    preventDefault() {},
  });
...
test('关闭动画结束后面板卸载，入口恢复淡入与拖动', async (t) => {
  ...
  await act(async () => {
    chat.fab().props.onPointerDown(chat.middleDown());
  });
  await act(async () => { chat.fab().props.onPointerMove({ ...{ clientX: 90, clientY: 80 }, pointerId: 1 }); });
  await act(async () => { chat.fab().props.onPointerUp({ pointerId: 1 }); });
...
test('打开面板取消进行中的拖动：dragging 类移除且后续 mousemove 不再生效', async (t) => {
  ...
  await act(async () => { chat.fab().props.onPointerMove({ ...{ clientX: 70, clientY: 60 }, pointerId: 1 }); });
  ...
  await act(async () => { chat.fab().props.onPointerMove({ ...{ clientX: 400, clientY: 400 }, pointerId: 1 }); });
  await act(async () => { chat.fab().props.onPointerUp({ pointerId: 1 }); });
...
test('面板打开时入口不响应拖动，重复点击不重复刷新联系人', async (t) => {
  ...
  await act(async () => { chat.fab().props.onPointerMove({ ...{ clientX: 250, clientY: 250 }, pointerId: 1 }); });
  await act(async () => { chat.fab().props.onPointerUp({ pointerId: 1 }); });
