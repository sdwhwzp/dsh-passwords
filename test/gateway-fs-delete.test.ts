...
function managedPaths(userId: number): string[] {
  const workspace = db.getManagedWorkspace(userId);
  return workspace === null ? [] : [normalizePath(workspace.path)];
}
...
async function followWorkspaceBaseline(cookie: string): Promise<any> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    client.on('close', (code: number, reason: Buffer) => { clearTimeout(timer); reject(new Error(`mux 在 baseline 前关闭: ${code} ${reason}`)); });
  });
  return client;
}
...
before(async () => {
  ...
  rootDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'dshpw-fsdel-root-')));
  ...
  upstream.on('request', (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/api/workspace.list') {
      void readJsonBody(req).then((body) => {
        res.writeHead(200, { 'content-type': 'application/json' });
        res.end(JSON.stringify({ type: 'server-response', rpcId: body.rpcId, result: { ok: true, value: { items: upstreamWorkspaces, archivedSessionIds: [] } } }));
      });
      return;
    }
  ...
  gateway = createGatewayServer(config, new AuthService(config, db), db, { upstreamBrowserCookie: process.env.DSH_UPSTREAM_AUTH_COOKIE });
...
test('主用户递归删除普通目录（含嵌套内容）并审计留痕', async () => {
  ...
  // 响应回显 realpath（macOS 的 /var 是 /private/var 的符号链接）。
  const realTarget = realpathSync(target);
  const res = await del(target);
  assert.equal(res.status, 200, JSON.stringify(res.json));
  assert.equal(res.json.ok, true);
  assert.equal(res.json.deleted, realTarget);
...
test('删除命中 workspace：rmSync 前快照、按 rc.2 信封删除注册表条目并清理 DB', async () => {
  ...
  db.setManagedWorkspace(syncUser.id, target);
  for (const id of ['sess-sync-1', 'sess-keep']) db.claimSessionOwner(id, syncUser.id);
  db.setPermissions(syncUser.id, {
  ...
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(target)), false);
  const perms = db.getPermissions(syncUser.id)!;
  assert.deepEqual(perms.allowed_folders, [keepFolder.replace(/\\/g, '/')]);
  assert.equal(perms.disabled_sessions.includes('sess-sync-1'), true);
  assert.equal(perms.disabled_sessions.includes('sess-keep'), false);
  assert.equal(db.getSessionOwner('sess-sync-1'), syncUser.id);
});
...
test('DB 清理不 fail-open：白名单删空必须回落 __deny__', async () => {
  ...
  const syncUser = db.getUserByUsername('sync-user')!;
  db.setManagedWorkspace(syncUser.id, target);
  for (const id of ['sess-deny']) db.claimSessionOwner(id, syncUser.id);
  db.setPermissions(syncUser.id, {
  ...
  assert.equal(db.getPermissions(syncUser.id)!.disabled_sessions.includes('sess-deny'), true);
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(target)), false);
});
...
test('未登记上游凭据：fail-closed，不转发浏览器 dsh-auth、不发任何上游请求', async () => {
  ...
  delete process.env.DSH_UPSTREAM_AUTH_COOKIE;
  const gatewayNoAuth = createGatewayServer(config, new AuthService(config, db), db, { upstreamBrowserCookie: process.env.DSH_UPSTREAM_AUTH_COOKIE });
...
test('删除父目录时按树内命中清理 DB 归属与白名单（树外保留）', async () => {
  ...
  const syncUser = db.getUserByUsername('sync-user')!;
  db.setManagedWorkspace(syncUser.id, parent);
  db.setPermissions(syncUser.id, {
  ...
  assert.equal(existsSync(parent), false);
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(parent)), false);
...
test('DB 清理失败：返回 DB_CLEANUP_FAILED、仍报告目录已删、失效 mux 且可重试收敛', async () => {
  ...
  const syncUser = db.getUserByUsername('sync-user')!;
  db.setManagedWorkspace(syncUser.id, target);
  for (const id of ['sess-dbfail']) db.claimSessionOwner(id, syncUser.id);
  db.setPermissions(syncUser.id, {
    allowedFolders: [target], hourlyTokenLimit: null, dailyMinutesLimit: null,
    allowUpload: false, allowGitDownload: false, allowWorkspaceCreate: true,
    banned: false, sandboxMode: null, disabledSessions: [],
...
test('清理意图持久化：DB 清理失败且仅剩会话 grants，模拟重启后重试仍能收敛', async () => {
  ...
  // 准入只能依赖持久化的清理意图。
  for (const id of ['sess-intent-only']) db.claimSessionOwner(id, syncUser.id);
  db.setPermissions(syncUser.id, {
  ...
  assert.equal(db.getPermissions(syncUser.id)!.disabled_sessions.includes('sess-intent-only'), false);
  assert.equal(db.getSessionOwner('sess-intent-only'), syncUser.id);
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(target)), false);
  ...
  const freshGateway = createGatewayServer(config, new AuthService(config, db), db, { upstreamBrowserCookie: process.env.DSH_UPSTREAM_AUTH_COOKIE });
  ...
  assert.equal(db.getPermissions(syncUser.id)!.disabled_sessions.includes('sess-intent-only'), true);
  assert.equal(db.getSessionOwner('sess-intent-only'), syncUser.id);
...
test('清理意图无法持久化：返回显式不可重试 code，不虚假宣称可重试', async () => {
  ...
  db.deleteManagedWorkspace(syncUser.id);
});
...
test('Windows 大小写不敏感：DB 中不同大小写的路径随目录删除清理', async (t) => {
  ...
  const syncUser = db.getUserByUsername('sync-user')!;
  db.setManagedWorkspace(syncUser.id, upper);
  db.setPermissions(syncUser.id, {
  ...
  assert.equal(res.status, 200, JSON.stringify(res.json));
  assert.equal(existsSync(target), false);
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(upper)), false);
  assert.deepEqual(db.getPermissions(syncUser.id)!.allowed_folders, ['__deny__']);
});
...
test('DB 别名清理：归属与白名单存别名时随真实目录删除清理，且重试准入与清理同口径', async (t) => {
  ...
  db.setManagedWorkspace(syncUser.id, aliasRow);
  db.setPermissions(syncUser.id, {
  ...
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(aliasRow)), true);

  // 重试：真实路径已不存在；准入必须凭别名行归位放行，DB 清理也必须命中同一别名行。
  const retry = await del(sub, adminBCookie);
  assert.equal(retry.status, 200, JSON.stringify(retry.json));
  assert.equal(retry.json.alreadyDeleted, true);
  assert.equal(managedPaths(syncUser.id).includes(normalizePath(aliasRow)), false);
  assert.deepEqual(db.getPermissions(syncUser.id)!.allowed_folders, [keep.replace(/\\/g, '/')]);
});
...
test('普通账号不能借目录创建获得授权树外的临时权限', async () => {
  const sub = db.getUserByUsername('subuser')!;
  const keep = path.join(rootDir, 'owned-create');
  const outside = path.join(rootDir, 'outside-create');
  mkdirSync(keep, { recursive: true });
  db.setManagedWorkspace(sub.id, keep);
  db.setPermissions(sub.id, {
    allowedFolders: [keep], hourlyTokenLimit: null, dailyMinutesLimit: null,
    allowUpload: true, allowGitDownload: false, allowWorkspaceCreate: true, banned: false,
  });
  const result = await post('/api/directoryPicker/createDirectory', {
    type: 'client-request', rpcId: 'outside-create', method: 'directoryPicker/createDirectory',
    payload: { args: { path: outside } },
  }, subuserCookie);
