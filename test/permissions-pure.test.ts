...
test('共享设置权限：写入口和敏感描述仅管理员可用', () => {
  ...
  for (const route of ['/api/dsh-skill-explorer/read', '/api/dsh-skill-explorer/update', '/api/dsh-context/balance']) {
    for (const method of ['GET', 'POST']) assert.equal(isAdminOnlyPluginEndpoint(method, route), true);
  }
...
test('alpha.1 agent-addressed endpoints inherit Session ownership', () => {
  ...
  assert.equal(SESSION_SCOPED_RE.test('/api/session/openWorkspacePath'), false);
  assert.equal(isSubuserBlockedApiPath('/api/session/openWorkspacePath'), true);
});
