...
test('热更新：未登记时 fail-closed，写入 .env 后无需重启即生效（两把钥匙都验证）', async () => {
  // 本 fork：未登记的第三方路径由账号隔离 Host 按签名 principal 处理，网关不 fail-closed。
  assert.equal(await request('/api/plugin/terminal', subCookie), 200, '未登记：子用户由 Host 侧处理');
...
test('热更新：owner: 前缀与传输前缀在运行中变更同样生效', async () => {
  ...
  await wait(SETTLE_MS);
  assert.equal(await request('/api/plugin/terminal', subCookie), 200, 'ws: 规则不作用于 HTTP 通道（未登记 → 由 Host 侧处理）');
...
test('热更新：规则被清空后立即收紧，且已授权 WebSocket 被断开（撤销语义）', async () => {
  ...
  assert.equal(await request('/api/plugin/terminal', subCookie), 200, 'HTTP 侧不再受登记表约束（未登记 → 由 Host 侧处理）');
});
