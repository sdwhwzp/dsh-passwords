...
官方终端 Remote 流 `terminal/follow`、`terminal/retain` 仍由账号 SSH 权限及会话归属控制；共享默认模型初始化和未知终端端点不因此放行。
...
纯会话保留模型选择、流式回复和历史记录，可使用账号已获授权的全部工具，包括知识库、GenUI 图表、文件、Shell 和子代理。服务器自动关联账号工作区；账号权限、沙盒及跨账号隔离与开发模式相同。实现与维护说明见 [纯会话模式](docs/2026-09-14-conversation-mode.md)。
...
- 管理页还支持新建文件夹，以及把文件/文件夹“移动”“复制”到另一个目录（先在条目上点移动或复制，进入目标目录后点“粘贴到当前目录”）；来源必须是普通文件或目录，目标必须在自己的托管目录内且尚不存在，因此不会覆盖已有文件，也不会经符号链接写出目录之外
- 管理页可直接 `git clone` 拉取代码，并对已克隆的目录执行 `git pull --ff-only`；只接受无查询参数的 http/https 地址；私有仓库可填写独立的用户名与密码／Token（GitHub 使用 PAT），需要同时开启“允许上传文件”和“允许 git 下载”。git 以最小环境运行：不读取宿主账号的 git 配置与凭据助手、禁用交互式提示、禁止 http(s) 以外的传输（含 `ext::` 这类可执行命令的传输），并把仓库里的符号链接写成普通文件；同一账号同时只允许一个 git 任务，超时 5 分钟，输出中的凭据会被打码。凭据只用于当次克隆或拉取，提交后清空输入框，不写入仓库地址或凭据存储；下次私有拉取需重新输入。认证请求不跟随重定向，请填写仓库最终地址。新克隆会拆出旧式 URL 中的凭据并保存无凭据的 origin；既有仓库中的历史凭据不会被批量迁移
...
- **工作区与会话权限**：主用户在每个子用户权限面板中用滑动开关开启工作区；开启后其中的活动会话默认全部可用，也可逐条取消勾选。子用户可以移除本人托管目录或本人配对本机目录的工作区登记，不能移除共享授权目录或其他账号的登记；删除只会把本人会话移入“未分组”，不会删除目录、文件或会话记录，目录仍获授权时可以继续读取。新建但尚未提问的空白会话也可分配，无标题时显示会话 ID；归档、已删除的会话及目录已失效的工作区不会出现在设置里，读取失败会明确提示重试。托管子账号没有额外工作区时只保留自己的专属目录，不会把空清单解释为全盘开放
...
- **客户模型范围**：子用户在 ChatGPT（Codex）服务商下显示 GPT-5.6 及以上版本（包含 GPT-6 Astra），其他服务商模型保持可用；服务端同时拒绝子用户调用低于 GPT-5.6 或无法识别版本的 Codex 模型，主用户不受限制
...
实时会话的权限校验直接读取 Host 内存中的会话头；只有不在内存中的会话才读取历史目录，且单次校验最多读取一次。每次校验仍重新检查账号、目录、会话所有权和禁用状态，子 Agent 只沿 Host 记录的父会话继承所有权。
...
登录 dsh 后，打开 **设置 → 密码门** 修改自己的登录信息与偏好。管理员可从侧栏或设置中的 **打开账号管理页** 进入独立页面 `/gateway/accounts`，按用户名或 ID 搜索，筛选角色和封禁状态，按账号名、最近登录或创建时间排序，并选择每页 25、50 或 100 个账号。列表展示月额度与本月消费，点击单个账号即可创建、改名、重置密码、删除或编辑权限；只有选中的权限表单会展开。页面可以收藏，仅当前有效管理员会话可以访问。
...
- **npm 装 dsh 报 allow-scripts / node-pty 错？** npm 新版会拦截安装脚本，先放行再重装：`npm config set allow-scripts=@deepseek-ai/dsh-subprocess-local,koffi,node-pty,@google/genai,protobufjs --location=user`，然后重新 `npm install -g @deepseek-ai/dsh@0.1.3-alpha.1`（本项目自身没这个问题，是 dsh 的依赖要跑原生构建）。
...
- **子用户隔离（第三方插件面）**：SSH 通过账号权限和独立连接存储隔离；skin-center、modlens、dsh-uploads 列表/删除等共享运维端点仅主用户可用。创意工坊的 `/api/preset-center` 及其子路径同样仅管理员可用，涵盖共享预设列表、组合源码、启用、停用和卸载；普通账号仍可列出并选择其获准使用的会话预设，SSH 端点授权不能放开全局预设管理。上传/下载按 `allow_upload` / `allowGitDownload` 权限门控，**新子用户默认禁 git 下载**（含 SSH 远程文件下载和 dsh-uploads 下载），主用户按需开启；账号隔离模式的 SSH 目录列表只要求 SSH 权限。better-sidebar 的普通文件预览与下载另行绑定到当前账号拥有的 Session 及其获准工作区，伪造 `sessionId`、`cwd` 或文件路径会返回 403；子用户的服务器文件由网关在授权后锁定同一文件描述符读取，路径并发替换不会把读取目标切换到其他账号的文件。
...
- 面向 DeepSeek Harness `0.1.3-alpha.1` 发布，运行时 peer 与类型编译依赖均精确固定到该版本；可选的 principal-access peer、安装提示与 Docker 默认版本同步对齐。
...
普通账号可通过 `workspaceFiles/changes` 订阅本人会话工作区的文件变化，打开或关闭交付文件预览不会中断同一连接上的历史流。网关按会话归属、允许目录和禁用状态校验订阅，并在转发变更时重新校验权限；无效或其他账号的会话请求会被拒绝。
...
配对目录的 `@` 索引、工作区文件预览与右侧文件面板通过已登录账号的本机助手读取文件，服务器占位目录只用于登记工作区。需要支持 `files` 操作的桌面端 0.1.2 或新版命令行助手，关闭 Shell 不影响读取。媒体下载与 HTML 预览同样绑定所属会话，再交由本机助手读取；HTML 文档保留原文与侧栏沙盒策略。客户端离线、目录撤销或账号不匹配时明确报错。普通服务器工作区仍使用原有文件接口。
