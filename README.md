...
左侧选择“开发模式”继续使用工作区、终端和编辑器；选择“纯会话”直接提问，不需要手动选择工作区。“新建纯会话”开启另一个话题，未提问的空会话由服务端按账号复用，多窗口同时新建也只创建一次。历史列表仅显示当前选中的空白会话；有消息、运行任务、排队输入、已归档或已命名的会话不会被自动清理。聊天记录随账号保存，重新进入时可以继续。
