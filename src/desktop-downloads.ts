...
interface Download { file: string; platform: 'windows-x64' | 'mac-arm64'; bytes: number; sha256: string; signing?: 'unsigned' | 'apple-notarized' }
...
export function registerDesktopDownloads(app: Express, directory: string | undefined, langOf: (req: Request) => Lang): void {
  ...
  for (const entry of catalog.files) {
    if (!entry || typeof entry.file !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9._-]*\.(exe|dmg|zip)$/.test(entry.file)
      ...
      || (entry.signing !== undefined && entry.signing !== 'unsigned' && entry.signing !== 'apple-notarized')
      || (entry.signing === 'apple-notarized' && entry.platform !== 'mac-arm64')
