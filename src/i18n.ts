...
const DICT: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    'gw.conversationModeDenied': '纯会话不支持此操作，请另开开发会话。',
    'gw.agentPresetDenied': '你的账号未被授权使用该 Agent preset，请联系主用户在「子用户权限 → Agent preset」里开通',
    'gw.sshHostDenied': 'SSH 地址被网络策略拒绝：目标为内网、回环或代理虚拟地址，请联系管理员配置可信 SSH 域名',
    'gw.sshHostUnresolved': '无法解析 SSH 主机地址，请检查域名或服务器 DNS 设置',
  ...
  en: {
    ...
    'gw.conversationModeDenied': 'This operation requires a separate development Session.',
    'gw.agentPresetDenied': 'Your account is not allowed to use that Agent preset. Ask the owner account to grant it under subuser permissions.',
    'gw.sshHostDenied': 'SSH address denied by network policy: the target is private, loopback, or proxy-assigned. Ask an administrator to configure the trusted SSH hostname.',
    'gw.sshHostUnresolved': 'Cannot resolve the SSH hostname. Check the hostname or server DNS settings.',
