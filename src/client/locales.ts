...
export const zh = {
  ...
  sess: '会话归属',
  sessHint: '升级前创建的旧会话没有归属记录，对子用户默认隐藏。把它们分配给子用户后，子用户即可看到并继续使用。',
  sessEmpty: '暂无未归属会话。若旧会话未显示，请先在主界面刷新一次会话列表。',
  sessFolder: '工作区',
  sessLastSeen: '最近活跃',
  sessAssign: '分配',
  sessAssignGroup: '将该工作区全部分配给',
  sessAssignFailed: '分配失败，请检查每条会话的错误原因',
  sessAssignPartial: '部分会话已分配，部分失败',
  sessTarget: '分配给',
  sessAssigned: '已分配',
  sessNoTarget: '请先选择目标用户',
  'sess.err.SESSION_UNKNOWN': '会话未被登记（请先刷新会话列表）',
  'sess.err.CWD_UNKNOWN': '会话工作区未知，无法校验目录权限',
  'sess.err.FOLDER_NOT_ALLOWED': '该会话所在目录不在目标用户的授权工作区内',
  'sess.err.ALREADY_OWNED': '该会话已归属其他用户（重分配需强制确认）',
  // 通用
  opFailed: '操作失败',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  // Session ownership (owner only, Discussion #6)
  sess: 'Session ownership',
  sessHint: 'Sessions created before the ownership tracking update are hidden from subusers by default. Assign them to a subuser to make them visible again.',
  sessEmpty: 'No unassigned sessions. If old sessions are missing, refresh the session list in the main UI first.',
  sessFolder: 'Workspace',
  sessLastSeen: 'Last seen',
  sessAssign: 'Assign',
  sessAssignGroup: 'Assign all in this workspace to',
  sessAssignFailed: 'Assignment failed; check each session for the reason',
  sessAssignPartial: 'Some sessions were assigned; some failed',
  sessTarget: 'Assign to',
  sessAssigned: 'Assigned',
  sessNoTarget: 'Select a target user first',
  'sess.err.SESSION_UNKNOWN': 'Session not registered (refresh the session list first)',
  'sess.err.CWD_UNKNOWN': 'Session workspace unknown, cannot verify folder permission',
  'sess.err.FOLDER_NOT_ALLOWED': 'The session workspace is not in the target user\'s allowed folders',
  'sess.err.ALREADY_OWNED': 'Session already owned by another user (force required to reassign)',
