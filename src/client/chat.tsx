...
export function ChatLauncher(props: PropsLocale<'dshpw'>) {
  ...
  const onFabPointerDown = (e: React.PointerEvent<HTMLButtonElement>) => {
    if (fabHidden || !e.isPrimary || (e.button !== 0 && e.button !== 1)) return;
...
const CHAT_CSS = `
.dshpw-chat-fab{position:fixed;z-index:2147483000;width:36px;height:36px;border-radius:50%;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);display:flex;align-items:center;justify-content:center;cursor:grab;touch-action:none;box-shadow:0 2px 8px rgba(0,0,0,.18);transition:transform .18s,box-shadow .18s,background .18s,opacity .18s,visibility .18s;pointer-events:auto;animation:dshpwFabIn .4s cubic-bezier(.34,1.56,.64,1)}
