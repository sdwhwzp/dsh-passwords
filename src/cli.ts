 ...
 */
const DSH_SUPPORTED_RUNTIME_RE = /^0\.1\.(?:(?:2|3|5|6)(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?|7-(?:alpha\.2|rc\.[12]))$/;
...
function runPatch(argv: string[]): void {
  ...
  if (action === undefined || action === 'on' || action === 'reload') {
    ...
    if (!isSupportedDshRuntime(dshVersion)) {
      console.error(`[dsh-passwords] Unsupported or invalid DSH version ${dshVersion ?? '(missing/corrupt)'}; refusing to patch (supported versions: 0.1.2, 0.1.3, 0.1.5, 0.1.6, 0.1.7-alpha.2, 0.1.7-rc.1, 0.1.7-rc.2)`);
...
async function boot() {
  ...
  const dshVersion = readDshVersion(root);
  if (!isSupportedDshRuntime(dshVersion)) {
    console.error(`[dsh-passwords] Unsupported or invalid DSH version ${dshVersion ?? '(missing/corrupt)'}; refusing to patch or start the public gateway (supported versions: 0.1.2, 0.1.3, 0.1.5, 0.1.6, 0.1.7-alpha.2, 0.1.7-rc.1, 0.1.7-rc.2)`);
    process.exit(EXIT_DSH_VERSION_UNSUPPORTED);
  }
  try {
    const result = applyRemotePatch(root);
    ...
    if (status.connectionCookieBridge !== 'native') {
