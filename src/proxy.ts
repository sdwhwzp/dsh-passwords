/**
 * Source proxy routes and sandbox requests. The private gateway reuses the sandbox
 * applier and owns its principal-aware proxy routes; it does not register both routers.
 */
...
export interface SandboxApplierDeps {
  /** Supplies the current private Host authentication headers for internal requests. */
  getAuthenticationHeaders?: () => Readonly<Record<string, string>>;
  upstreamTransport: typeof https | typeof http;
  upstreamHost: string;
  upstreamPort: number;
  internalSecret: string;
...
function filterByPathFieldWithPredicate(
  value: unknown,
  field: string,
...
function replaceArchivedSessionSnapshot(target: Set<string>, value: unknown): boolean {
...
const MAX_ARCHIVED_SESSION_IDS = 10_000;
